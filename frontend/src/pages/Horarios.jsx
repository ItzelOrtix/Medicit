  ...
  return (
    <div className="p-4 sm:p-8">
      <div className="flex items-center justify-between mb-6 sm:mb-8">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">Horarios</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Disponibilidad de los médicos por día</p>
        </div>
        <Button onClick={() => setModal(true)}>
          <Plus size={16} /> <span className="hidden sm:inline">Agregar horario</span>
        </Button>
      </div>
      ...
      <div className="flex items-center gap-2 mb-6 flex-wrap">
        ...
        {medicos.map((m) => (
          <button
            ...
            className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-colors cursor-pointer ${
              String(m.id) === medicoSeleccionado
                ? 'bg-gray-900 text-white dark:bg-white dark:text-gray-900'
                : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700'
            }`}
          >
      ...
      <div className="flex flex-col gap-4">
        {filteredMedicos.map((medico) => {
          ...
          return (
            <div key={medico.id} className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
              <div className="flex items-center gap-3 px-4 sm:px-6 py-4 border-b border-gray-50 dark:border-gray-700">
                <div className="w-9 h-9 rounded-xl bg-gray-100 dark:bg-gray-700 flex items-center justify-center text-xs font-bold text-gray-600 dark:text-gray-300 shrink-0">
                  {medico.nombre[0]}{medico.apellido[0]}
                </div>
                <div>
                  <p className="font-semibold text-gray-900 dark:text-white text-sm">{medico.nombre} {medico.apellido}</p>
                  <p className="text-xs text-gray-400 dark:text-gray-500">{medico.especialidades?.[0]?.nombre || 'Sin especialidad'}</p>
                </div>
                <span className="ml-auto text-xs text-gray-400 dark:text-gray-500 flex items-center gap-1 shrink-0">
                  <Clock size={12} /> {horas.length} horarios
                </span>
              </div>

              {horas.length === 0 ? (
                <p className="text-sm text-gray-400 dark:text-gray-500 text-center py-6">Sin horarios definidos</p>
              ) : (
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7 gap-0 divide-x divide-gray-50 dark:divide-gray-700">
                  {DIAS.map((dia) => {
                    const diaHoras = horas.filter((h) => h.diaSemana === dia);
                    return (
                      <div key={dia} className="p-3 sm:p-4 min-h-[100px]">
                        <p className="text-xs font-semibold text-gray-400 dark:text-gray-500 mb-2">{DIA_LABEL[dia]}</p>
                        <div className="flex flex-col gap-2">
                          {diaHoras.map((h) => (
                            <div key={h.id} className={`rounded-lg p-2 text-xs border ${h.disponible ? 'bg-gray-50 dark:bg-gray-700/60 border-gray-100 dark:border-gray-600' : 'bg-white dark:bg-gray-800 border-gray-100 dark:border-gray-700 opacity-50'}`}>
                              <p className="font-medium text-gray-700 dark:text-gray-300">{h.horaInicio}–{h.horaFin}</p>
                              <div className="flex items-center justify-between mt-1.5 gap-1">
                                <button onClick={() => handleToggle(h.id)} className="text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 transition-colors cursor-pointer">
                                  {h.disponible
                                    ? <ToggleRight size={16} className="text-green-500 dark:text-green-400" />
                                    : <ToggleLeft size={16} className="dark:text-gray-500" />}
                                </button>
                                <button onClick={() => handleDelete(h.id)} className="text-gray-300 dark:text-gray-600 hover:text-red-400 dark:hover:text-red-400 transition-colors cursor-pointer">
                                  <Trash2 size={12} />
                                </button>
                              </div>
                            </div>
