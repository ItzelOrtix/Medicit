...
export default function DoctorHorario() {
  ...
  useEffect(() => {
    ...
    ]).then(([citasRes, horariosRes]) => {
      const misCitas = citasRes.data.filter((c) => c.medicoId === medicoId);
      setCitas(misCitas);
      setHorarios(horariosRes.data || []);

      // Extrae pacientes únicos de las citas del médico
      const vistos = new Set();
      const pacientesUnicos = [];
      for (const c of misCitas) {
        if (!vistos.has(c.pacienteId)) {
          vistos.add(c.pacienteId);
          pacientesUnicos.push({
            id:       c.pacienteId,
            nombre:   c.paciente?.nombre    || c.pacienteNombre    || '–',
            apellido: c.paciente?.apellido  || c.pacienteApellido  || '',
          });
        }
      }
