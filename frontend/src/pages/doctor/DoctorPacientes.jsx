    ...
    citaService.getAll()
      .then((citasRes) => {
        ...
        const enriched = pacienteIds.map((pid) => {
          const citaRef = misCitas.find((c) => c.pacienteId === pid);
          const citas   = misCitas.filter((c) => c.pacienteId === pid)
                                  .sort((a, b) => b.fecha.localeCompare(a.fecha));
          return {
            id:         pid,
            nombre:     citaRef?.paciente?.nombre    || citaRef?.pacienteNombre    || '–',
            apellido:   citaRef?.paciente?.apellido  || citaRef?.pacienteApellido  || '',
            email:      citaRef?.paciente?.correo    || '',
            telefono:   citaRef?.paciente?.telefono  || '',
            genero:     citaRef?.paciente?.genero    || '',
